...
//import com.petguardian.admin.model.AdminRepository;
...
//    private AdminRepository adminRepository;
    ...
    public Map<String, Object> getDashboardStatistics() {
        ...
        // 1. 管理員總數 (TABLE admin: adm_status = 1 啟用)
//        long totalAdmins = adminRepository.countByAdmStatus(1);
//        stats.put("totalAdmins", totalAdmins);

        // 2. 會員總數 (TABLE member: mem_status = 1 啟用)
        long totalMembers = memberRepository.countByMemStatus(1);
        stats.put("totalMembers", totalMembers);

        // 3. 待審保母 (TABLE sitter_application: app_status = 0 待審核)
        long pendingSitters = sitterApplicationRepository.countByAppStatus((byte) 0);
        stats.put("pendingSitters", pendingSitters);

        // 4. 預約待審退款 (TABLE booking_order: order_status = 3 申請退款中)
        ...
        // 5. 預約待審評價檢舉 (TABLE booking_order_report: report_status = 0 未處理)
        ...
        // 10. 論壇待審評價(留言)檢舉 (TABLE forumcommentreport: report_status = 0 待審核)
        ...
        // 11. 論壇待審文章檢舉 (TABLE forumpostreport: report_status = 0 待審核)
