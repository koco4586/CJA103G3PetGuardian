...
public class PetRepository {
    ...
    private static final String INSERT =
        "INSERT INTO PET (MEM_ID, TYPE_ID, SIZE_ID, PET_NAME, PET_GENDER, PET_AGE, PET_DESCRIPTION, PET_IMAGE, CREATED_TIME) " +
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())";

    private static final String UPDATE =
        "UPDATE PET SET MEM_ID=?, TYPE_ID=?, SIZE_ID=?, PET_NAME=?, PET_GENDER=?, PET_AGE=?, PET_DESCRIPTION=?, PET_IMAGE=? WHERE PET_ID=?";
    ...
    private static final String GET_ALL =
        "SELECT p.*, pt.TYPE_NAME, ps.SIZE_NAME " +
        "FROM PET p " +
    ...
    private static final String GET_BY_MEM_ID = 
    	    "SELECT p.* FROM PET p WHERE p.MEM_ID = ? ORDER BY p.CREATED_TIME DESC";
    ...
    private final RowMapper<PetVO> petRowMapper = new RowMapper<PetVO>() {
        ...
        public PetVO mapRow(ResultSet rs, int rowNum) throws SQLException {
            ...
            pet.setCreatedTime(rs.getTimestamp("CREATED_TIME"));
